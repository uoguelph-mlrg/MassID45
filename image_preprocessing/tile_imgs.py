...
base_dir = "/h/jquinto/lifeplan_b_v9_cropped_center"
...
def get_sahi_coco_data(crop_fac, crop_rows, 
  ...
  sahi_file_path = f"{sahi_file_path}/sahi_{crop_fac}_ignore_neg"
...
# Scott's analysis with different crop sizes
# ZOOM_FACTORS = list(range(1, 5)) + [6, 8]
ZOOM_FACTORS = [2]

BASE_SIZE = 1024
crop_zoom_sizes = [int(BASE_SIZE/zoom_factor) for zoom_factor in ZOOM_FACTORS]
for sz, zoom_fac in zip(crop_zoom_sizes, ZOOM_FACTORS):
  print(f"Zoom Factor: {zoom_fac} | Crop Size: {sz}")

for crop_sz in crop_zoom_sizes:
